...
class SubimageCreator:
    ...
    def cut(self):
        ...
        for i in range(0, image.shape[0], self.size[0]):
            for j in range(0, image.shape[1], self.size[1]):
                ...
                # Saving the subimage
                cv2.imwrite(os.path.join(self.output_dir, subimage_name), subimage)
    ...
    def rebuild(self,prefix : str = ""):
        ...
        number_of_subimage_x=int(self.original_image_size[0]/self.size[0]) if self.original_image_size[0]%self.size[0]==0 else int(self.original_image_size[0]/self.size[0])+1
        number_of_subimage_y=int(self.original_image_size[1]/self.size[1]) if self.original_image_size[1]%self.size[1]==0 else int(self.original_image_size[1]/self.size[1])+1
        ...
        matrix_image=np.zeros((length_image,height_image,3),dtype=np.uint8)
